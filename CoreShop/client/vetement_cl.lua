...
function SavePlayerSkin()
    TriggerEvent('skinchanger:getSkin', function(skin)
        ...
        TriggerServerEvent('marseille:savechapeau', skin)
        TriggerServerEvent('marseille:savelunettes', skin)
        TriggerServerEvent('marseille:saveoreillette', skin)
        TriggerServerEvent('marseille:savemasque', skin)
    end)
end
    ...
    Events = {
        ...
        onSelected = function(self, _, btn, PMenu, menuData, currentButton, currentBtn, currentSlt, result, slide)
            ...
            elseif btn == "Payer" then
                SavePlayerSkin()
                TriggerServerEvent('marseille:buycloth')
...
local VetementNoPed = {
    ...
    Events = {
        ...
        onSelected = function(self, _, btn, PMenu, menuData, currentButton, currentBtn, currentSlt, result, slide)
		    ...
		    elseif btn == "Mes Tenues" then 
				ESX.ShowAdvancedNotification("~h~~r~Magasin Vetement ", "", "Pour accéder a votre garde-robe vous devez aller a la ~g~cabine d'essayage !", "CHAR_CHENG", 1)
            end
        end,
        onSlide = function(menuData, btn, currentButton, currentSlt, slide, PMenu)
            local currentMenu, currentBtn, slide, btn, ped = menuData.currentMenu, menuData.currentBtn, btn.slidenum, btn.name, GetPlayerPed(-1)
...
local BarberNoPed = {
    ...
    Events = {
        ...
        onSlide = function(menuData, btn, currentButton, currentSlt, slide, PMenu)
		    ...
		    elseif btn == "Mèche" and slide ~= -1 then
                teinture1 = slide - 1
                TriggerEvent('skinchanger:change', 'hair_color_2', teinture1)
...
Citizen.CreateThread(function()
    while true do
        ...
        for k, v in pairs(barberpoz) do
            ...
            if dist <= 2.0 then
                ESX.ShowHelpNotification("Appuyez sur ~INPUT_TALK~ pour accéder au ~b~coiffeur~w~.")
                if IsControlJustPressed(1,51) then 	
				FreezeEntityPosition(GetPlayerPed(-1), true)
                    TriggerEvent('skinchanger:getSkin', function(skin)
                        if skin.sex > 1 then
